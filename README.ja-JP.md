...
### 方法 1 — リンクを AI アシスタントに渡す（最も簡単）
以下のメッセージをコピーして、AI アシスタント（WorkBuddy、Claude、ChatGPT など）に貼り付けてください：

> このプロジェクトをセットアップしてください：https://github.com/BerryUIKI/Axiara
> 1. コードを取得して初期化してください——日本語で設定（言語・保存方法・データソース）を案内してください。
> 2. 準備ができたら、何ができるか教えてください。

あとは聞かれた質問に答えるだけです。それで完了です。

### 方法 2 — 自分でファイルをダウンロードしてから AI に渡す
1. このページの緑の **Code** ボタン → **Download ZIP**（または Releases のアーカイブがあればそれを利用）。
2. 解凍したフォルダをコンピュータの任意の場所に置きます。
3. AI アシスタントでそのフォルダを開き、「このプロジェクトを初期化して、セットアップを案内してください」と伝えます。
4. 質問に答えるだけです——完了。
