...
- **D-SK9** — Upload flow & data-repo branch rules (2026-08-05): triggers **"上传数据 / 重新上传 / 提交数据"** → export bundle named by **date + unique machine code** (`learn_inbox/<machine-id>/<yyyymmdd>/bundle.yaml`). Text data repo: **`main` protected (admin PR-only) · one `user/<machine-id>` branch per user (their only write point) · optional `review/<date>`**; users never push to `main`. SQL mode: identity is a column (`contributor`), no branches.
