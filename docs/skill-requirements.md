...
- **D-SK9** — Upload flow & data-repo branch rules (2026-08-05): triggers **"upload my data" / "re-submit" / "submit my library"** (localized equivalents matched by the Agent) → export bundle named by **date + unique user-id** (`learn_inbox/<user-id>/<yyyymmdd>/bundle.yaml`). Text data repo: **`main` protected (admin PR-only) · one `user/<user-id>` branch per user (their only write point) · optional `review/<date>`**; users never push to `main`. SQL mode: identity is a column (`contributor`), no branches. **Branch strategy user-selectable at team setup — default A: per-user branches (isolation/audit; anti-mess: stable user-id, append-only); option B: single `upload/` branch + directory isolation. Scale: >30 active contributors → Git not recommended, switch to SQL** (`learn-sync.md` §10).
- **D-SK10** — Dynamic scale monitoring (2026-08-05): static thresholds are a baseline only; the central training Agent runs a **floating check** (weekly + monthly health report) over rolling-90-day metrics (active contributors, upload volume, review backlog, branch/file sprawl, friction events) and maps to tiers 🟢 healthy / 🟡 watch / 🟠 upgrade advised / 🔴 must migrate, with migration paths (Git A → B → SQL, rolling switch optional). Suggestions auto-surfaced; **migration needs explicit human confirmation**. Report = chat + `output/` (like D-SK6).
- **D-SK11** — Inactive-branch archiving (2026-08-05): **off by default, enabled by admin in training mode** (`enable_branch_archive`). A `user/<user-id>` branch idle > 180 days (floating, configurable) → training Agent lists candidates → **admin confirms** → PR merges the branch into the single **`archive/` branch** (`archive/<user-id>/<date>/`, append-only, not merged into `main`) → delete the user branch → ledger. Reactivation recreates the user branch (optionally seeded from archive). SQL mode: not applicable (no branches).
